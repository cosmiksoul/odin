...
> 🎯 **MVP разбит на два этапа** (ADR-016, 2026-04-27):
> - **MVP-1** — read-only Catalog (M1 + M2 + опционально M3 + частично M4). Промежуточная веха ~1 месяц от текущей точки.
> - **MVP-2** — full ODIN: WYSIWYG, sync, LLM, Хугин (M-CRED + M-SYNC + M-EDIT + M5-M9). Финальная цель ~3-4 месяца после MVP-1.
>
> Внутренние milestones M1-M9 остаются как есть, добавлено только разделение по точке публикации/демонстрации. ADR-011 (WYSIWYG обязателен в MVP) уточнён: обязателен в **MVP-2**, не в MVP-1.
